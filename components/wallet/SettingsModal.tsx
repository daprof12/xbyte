...
import { generateAddressForCoin } from '../../utils/addressGenerator';
...
export default function SettingsModal({ walletData, onClose, onLogout, onUpdateWallet }: SettingsModalProps) {
  ...
  const isAssetVisible = (symbol: string) => {
    return !(walletData.hiddenAssets || []).includes(symbol);
  };

  const handleToggleAssetVisibility = (symbol: string) => {
    const currentHidden: string[] = walletData.hiddenAssets || [];
    const newHidden = currentHidden.includes(symbol)
      ? currentHidden.filter(s => s !== symbol)
      : [...currentHidden, symbol];
    ...
    dataService.setItem('xbyte_wallet', JSON.stringify(updatedWallet));
    window.dispatchEvent(new CustomEvent('walletUpdated', { detail: { walletData: updatedWallet } }));
  };
  ...
  const chains = loadAssetConfig().map(asset => ({
    ...
    network: asset.network || (
             asset.symbol === 'BTC' ? 'Bitcoin Mainnet' : 
      ...
      <div>
        <Tabs defaultValue="profile" className="w-full">
          ...
          <TabsContent value="addresses" className="space-y-4 mt-4">
            <div className="bg-gray-50 dark:bg-gray-700/60 p-4 rounded-xl border border-gray-200/80 dark:border-gray-600/70 mb-1">
            ...
            {chains.map((chain) => {
              const address = walletData.addresses?.[chain.symbol] || generateAddressForCoin(chain.symbol);
              const visible = isAssetVisible(chain.symbol);

              return (
                <div key={chain.symbol} className="p-4 bg-gray-50 dark:bg-gray-700 rounded-xl border border-gray-200/60 dark:border-gray-600/60">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      {chain.logoUrl ? (
                        <img src={chain.logoUrl} alt={chain.name} className="w-10 h-10 rounded-full object-cover" />
                      ) : (
                        <div className={`w-10 h-10 rounded-full ${chain.color} flex items-center justify-center text-white text-lg font-bold`}>
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">{chain.network}</div>
                      </div>
                    </div>

                    <div className="flex items-center gap-2.5">
                      <span className={`text-xs font-medium ${visible ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400 dark:text-gray-500'}`}>
                      ...
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={visible}
                          onChange={() => handleToggleAssetVisibility(chain.symbol)}
                  ...
                  <div className="flex gap-2 mt-3">
                    <div className="flex-1 p-2 bg-white dark:bg-gray-800 rounded text-xs font-mono text-gray-900 dark:text-white break-all border border-gray-200 dark:border-gray-700 select-all">
                      {address}
                    </div>
                    <Button
                      variant="outline"
